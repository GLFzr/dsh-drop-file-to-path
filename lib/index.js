...
/**
 * Dedupe lookup: is this exact file (name + byte size) already in the dropbox?
 * Binary uploads land as `name.b64` whose on-disk size is the base64 length
 * (4 × ceil(size/3)); text uploads land as `name` with the original size.
 * @param dir - dropbox directory.
 * @param name - sanitized original file name.
 * @param size - original byte size.
 * @returns the existing on-disk path + encoded flag, or null when absent.
 */
function findReusable(dir, name, size) {
  for (const fileName of [name, name + '.b64']) {
    ...
    const expected = fileName.endsWith('.b64') ? Math.ceil(size / 3) * 4 : size
    if (info.size === expected) return { path, encoded: fileName.endsWith('.b64') }
  }
  return null
}
  ...
  const routes = [
    {
      ...
      handler: (req, res) => handle(req, res, (args) => {
        ...
        // Reuse: an identical dropbox file (same name, same bytes) already
        // exists — return its path instead of creating a new copy, so
        // repeated drops of the same file never accumulate _1/_2 duplicates.
        const existing = findReusable(dropboxDir, name, size)
