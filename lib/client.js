...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    async function upload(file) {
      ...
      // Fast-path reuse: the host found a same-name same-size dropbox file
      // and returned its path — no chunked upload needed (instant re-drop).
    ...
    function ShellDrop() {
      ...
      if (pendingItems.length > 0) {
        ...
        const card = [
          React.createElement('div', { key: 't', className: 'dsh-dropbox-ready-title' }, '文件已就绪（' + pendingItems.length + ' 个）· v1.4.8'),
     ...
     * routes (/api/drop-file-to-path/list|clean), no composer involvement.
     */
    function fmtBytes(n) {
    ...
    function DropboxCleanupSection() {
      ...
      const load = () => {
        ...
        fetch('/api/drop-file-to-path/list', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: '{}',
        }).then((r) => r.json()).then((j) => {
      ...
      const runClean = () => {
        ...
        fetch('/api/drop-file-to-path/clean', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
